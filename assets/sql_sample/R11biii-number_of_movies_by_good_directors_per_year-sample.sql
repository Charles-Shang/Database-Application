-- R11iii - Number of movies directed by directors with top 10% relative rating for each year
...
(
SELECT director_id FROM DirectorRating WHERE relative_rank <= (SELECT COUNT(*) FROM DirectorRating)*0.1
)
GROUP BY year
ORDER BY year;